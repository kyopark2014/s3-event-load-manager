...
이러한 event driven architecture는 유연한 시스템을 구성하는데 많은 도움을 주지만, 프로세싱을 하는 Step Functions으로 인입되는 트래픽을 정밀하게 제어하기 어렵습니다. 예를 들면, S3로 인입되는 다수의 Data 처리를 한꺼번에 Step Functions에서 처리할 수 없는 경우에 50개 또는 100개 단위로 1분간격으로 처리하고자 한다면, 로드 메니저를 두어서 스케줄링을 하여야 합니다.
