...
5) Lambda(schedular)는 SQS(event)에서 N개의 메시지를 읽어오고, 읽어온 메시지는 삭제합니다. 여기서 N은 StepFunctions이 처리하는 그룹 작업(job)의 숫자입니다.
...
이러한 event driven architecture는 유연한 시스템을 구성하는데 많은 도움을 주지만, 프로세싱을 하는 Step Functions으로 인입되는 트래픽을 정밀하게 제어하기 어렵습니다. 예를 들면, Step Functions에서 원할히 처리되도록 50개 또는 100개 단위의 event를 1분 간격으로 처리하고자 한다면, load manager를 두어서 스케줄링을 하여야 합니다.
...
Lambda(invoke)는 SQS(invokation)이 전달해온 event에서 bucket의 이름과 key를 추출하여 이용하고, SQS(invokation)의 메시지를 삭제합니다.
...
[deployment.md](./deployment.md)에 따라 인프라를 설치하고 필요한 셈플 파일을 다운로드 합니다. 설치가 다 완료가 되면 아래와 같이 파일 복사 명령어를 확인할 수 있습니다.
...
### 실행 결과
CloudWatch에서 메시지가 순차적으로 처리되고 있는지 확인할 수 있습니다. 또한, SQS의 Monitoring에서 아래와 같이 인입된 메시지와 Lambda(invoke)로 전달된 메시지의 숫자를 확인할 수 있습니다.
...
또한, 아래와 같이 Load Manager를 이용함으로 인해, SQS(invokation)에 put event가 50개씩 전달된것을 확인할 수 있습니다. 15:48분과 15:49분에 각각 50개가 전달되었고, 이후 나머지 메시지들이 15:51분에 전달되었습니다. 이와같이 Step Functions에 전달되는 전체적인 부하(job)은 1분 간격으로 50이하로 관리할 수 있습니다. 
...
1분 간격으로 50개씩 부하(job)가 분배되었음을 확인할 수 있습니다.
